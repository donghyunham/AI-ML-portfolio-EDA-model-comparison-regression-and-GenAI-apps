...
class MeetingSummaryAgent:
    ...
    """
    def __init__(self, region_name="us-east-1", model_id="anthropic.claude-3-haiku-20240307-v1:0"):
    ...
    def _build_rag_prompt(self, context_chunks: list, query: str) -> str:
        ...
        return f"""
You are an executive AI Meeting Assistant utilizing Retrieval-Augmented Generation (RAG).
    ...
    def process_transcript(self, transcript: str, query: str = "회의 요약 및 액션 아이템 추출") -> tuple:
        ...
        if self.bedrock:
            try:
                ...
                return result["content"][0]["text"], retrieved_chunks
            except Exception:
...
- Data Leakage 차단을 위한 Scikit-learn Pipeline 모듈화 검증 공유
- AWS Bedrock Claude 3 하이쿠 기반 RAG 에이전트 구축 및 테스트 진행
